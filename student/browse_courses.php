    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      foreach ($courses as $course) {
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 h-full flex flex-col">
          <img src="'.$course['img'].'" class="w-full h-48 object-cover" alt="'.$course['title'].'" 
            onerror="this.onerror=null; this.src=\'https://via.placeholder.com/300x150?text=Course\';">
          <div class="p-4 flex-grow flex flex-col">
            <h5 class="text-lg font-bold text-blue-800 truncate mb-2">'.$course['title'].'</h5>
            <p class="text-gray-600 mb-4">'.$course['videos'].' Videos</p>
          </div>
          <div class="px-4 py-3 bg-gray-50 flex justify-between items-center">
            ...
            <a href="course_player.php?course='.urlencode($course['title']).'" class="bg-blue-800 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm font-medium transition-colors">
              Enroll now
            </a>
          </div>
        </div>';
